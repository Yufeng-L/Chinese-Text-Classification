    ...
    for eachDir in fatherLists:  # 遍历主目录中各个文件夹
        ...
        for eachFile in childLists:  # 遍历每个文件夹中的子文件
          ...
          #  print(eachFile)
            ...
            cutResult = jieba.cut(result,cut_all = False)  # 精确方式分词
...
def bayesAlgorithm(trainPath, testPath):
    ...
    total_prediction = len(predicted)
    error_prediction = 0
    for flabel, fileName, expct_cate in zip(testSet.label, testSet.filenames, predicted):
        if flabel != expct_cate:
            error_prediction += 1
            print(fileName, ":实际属性：", flabel, "-->预测属性：", expct_cate)
    print("")
    print("Error rate:", float(error_prediction) * 100 / float(total_prediction), "%")
